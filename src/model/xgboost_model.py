...
def get_data():
    ...
    train_feartue = pd.read_csv('../../data/for_add_feature/train_34_time.csv', header = None, index_col = None).as_matrix()
...
def build_loadfile():
    ...
    #feature
    submite_x_test_feature = pd.read_csv('../../data/for_add_feature/test_34_time.csv', header = None, index_col = None).as_matrix()
    ...
    pd.DataFrame(tofile).to_csv('testY3.csv', header=['USRID\tRST'], index = False)
